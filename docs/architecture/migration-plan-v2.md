...
| G | 部分完成（已达可集成基线） | `58fc4c5`、`a0fb0dc`、`47e9251`、本证据提交 | 公共入口、平台窄口、零深导入例外、27 张 Chrome 证据；Piece 中央注册表与全实体 StatBlock 留作后续窄迁移 |
...
- `58fc4c5` 完成 composition root、Runtime/App Shell、兼容门面与测试编排收口；`a0fb0dc` 固化文档、CI 和多人维护流程。
- Chrome 首轮复验发现 `render.js` 浮字表现缺少 `gamePack` 上下文，先以失败测试复现，再由 `47e9251` 修复；修复后重新创建标签并从头采集，未沿用故障截图。
- 最终运行 `chrome-modular-foundation-final-20260720` 共 27 张真 Chrome 截图，控制台 `error=0`、`warning=0`，源码指纹为 `6c46d57941141b8aab4fd994b8fe1b925b8bf7806d2cc073dc03e5d6abd4db43`。证据位于 `test-artifacts/screenshots/2026-07-20/`。
- 五关、批量征兵、营满不扣费、部分成功、铲地、交换、非法回滚、合成后移动、五英雄技能、Boss、战败重试与 390×844 视口均有状态清单和截图。
- 为缩短真实浏览器等待，波次通过生产调试句柄 `__step(0.025)` 推进；输入、规则、主循环、渲染和结算仍走生产链路，未使用 mock。合成截图只在隔离临时状态中准备一个重复字牌，实际合成/移动由页面拖拽和正式 GameCommand 完成，规则另由自动测试覆盖。
